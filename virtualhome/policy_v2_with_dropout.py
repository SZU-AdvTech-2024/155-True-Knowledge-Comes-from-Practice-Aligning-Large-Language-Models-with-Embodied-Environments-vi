...
#-----------------Addition-----------------
from critic_with_dropout import CriticWithDropout # 增加 CriticWithDropout 的引入
#------------------------------------------
...
class LLMAgent(nn.Module):
    ...
    def _init_critic(self, critic_weights = None):
        ...
        #-----------------Addition-----------------
        critic = CriticWithDropout(self.actor, self.tokenizer)
        #------------------------------------------
    ...
    def get_value(self, x):
        ...
        #---------Addition---------
        with self.actor.disable_adapter():
            value, uncertainty = self.critic.predict_with_uncertainty(
                input_ids, attention_mask, mc_samples=10
            )
    ...
    def get_action_and_value(self, obs, action=None, is_warmup=False, return_value = True):
        ...
        if return_value:
            ...
            #-----------------Addition-----------------
            value, uncertainty = self.get_value(prompt)
            return action, log_probs, entroy, value, uncertainty
            #------------------------------------------
        else:
            # return action, log_probs, entroy, None
            return action, log_probs, entroy, None, None
